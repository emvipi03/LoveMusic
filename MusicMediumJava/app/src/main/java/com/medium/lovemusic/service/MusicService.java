...
public class MusicService extends Service implements MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener {
    ...
    private void nextSong() {
        if (mRepeatMode != Constant.REPEAT_ONE) {
            if (mListSongPlaying.size() > 1 && mSongPosition < mListSongPlaying.size() - 1) {
                if (mRepeatMode == Constant.REPEAT_NONE) {
                    mSongPosition++;
                } else if (mSongPosition == mListSongPlaying.size() - 1 && mRepeatMode == Constant.REPEAT_ALL) {
                    mSongPosition = 1;
                }
            } else {
                mSongPosition = 0;
    ...
    public void onCompletion(MediaPlayer mp) {
        ...
        if (mRepeatMode == Constant.REPEAT_ONE) {
            // For REPEAT_ONE, just play the same song again
            playSong();
        } else {
            nextSong();
        }
    ...
    public static void toggleRepeatMode() {
        switch (mRepeatMode) {
            case Constant.REPEAT_NONE:
                mRepeatMode = Constant.REPEAT_ALL;
                break;
            case Constant.REPEAT_ALL:
                mRepeatMode = Constant.REPEAT_ONE;
                break;
            case Constant.REPEAT_ONE:
                mRepeatMode = Constant.REPEAT_NONE;
                break;
            // Add more cases if needed
        }

        // Update UI or send broadcast to notify about the repeat mode change
        sendBroadcastWithRepeatMode();
    }

    public static int getRepeatMode() {
        return mRepeatMode;
    }
    private static void sendBroadcastWithRepeatMode() {
        Intent intent = new Intent(Constant.REPEAT_MODE);
        intent.putExtra(Constant.REPEAT_MODE, mRepeatMode);
